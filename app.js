(function () {
  ...
  async function voteForCandidate(candidateId) {
    ...
    try {
      ...
      toast("投票できませんでした", error.message || "もう一度お試しください。");
    }
  }
  ...
  async function createFirestoreVote(categoryId, candidateId) {
    ...
    const userRef = firestore.doc(db, firebase.COLLECTIONS.users, user.uid);
...
(function () {
  ...
  function renderCandidates() {
    const selected = getCategory(appState.selectedCategory);
    const latestVote = getLatestUserVote();
    ...
    if (!appState.votingPeriodConfigured) {
      ...
      els.voteNotice.textContent = `次の投票まで ${formatDuration(
        cooldownRemaining
      )}。前回: ${getCandidate(latestVote.candidateId).username}`;
    } else {
      els.voteNotice.textContent = `${selected.name} に投票できます。投票は1時間に1度までです。`;
    }
      ...
      return `
        ...
        candidate.username
          ...
          <div class="candidate-footer">
            ...
            <button class="btn btn-primary vote-btn" type="button" data-candidate="${candidate.id}" ${
              disabled ? "disabled" : ""
            }>${recentlyVoted ? "VOTED" : buttonLabel}</button>
          </div>
        </article>
      `;
    }).join("");
  ...
  function renderProfile() {
    ...
    const votes = appState.votes.filter((vote) => vote.userId === user.uid).length;
    els.profileName.textContent = user.displayName;
    els.profileDescription.textContent = `${votes} 部門に投票済み。奈落鯖アワードのリアルタイム集計に接続中。`;
    els.profileAvatar.src = user.photoURL;
  }
  ...
  async function voteForCandidate(candidateId) {
    ...
    if (cooldownRemaining > 0) {
      toast("投票クールダウン中です", `次の投票まで ${formatDuration(cooldownRemaining)}。`);
      return;
    }
  ...
  async function createFirestoreVote(categoryId, candidateId) {
    ...
    const db = firebase.db;
    const user = appState.currentUser;
    const voteRef = firestore.doc(firestore.collection(db, firebase.COLLECTIONS.votes));
    const userRef = firestore.doc(db, firebase.COLLECTIONS.users, user.uid);

    await firestore.runTransaction(db, async (transaction) => {
      const userSnap = await transaction.get(userRef);
      const lastVoteAt = userSnap.exists() ? userSnap.data().lastVoteAt : null;

      if (lastVoteAt && Date.now() - getTime(lastVoteAt) < ONE_HOUR_MS) {
  ...
  function getLatestUserVote() {
    ...
    return (
      appState.votes
        .filter((vote) => vote.userId === appState.currentUser.uid)
        .sort((a, b) => getTime(b.createdAt) - getTime(a.createdAt))[0] || null
    );
  }
  ...
  function getVoteCooldownRemaining() {
    ...
    const elapsed = Date.now() - getTime(latestVote.createdAt);
